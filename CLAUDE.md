...
- **Always offer a paste slot for secrets via "Other".** When the user picks "Configure <KEY>", the very next `AskUserQuestion` MUST include a **"Paste value via \"Other\" ↓"** option as the first/recommended choice. `AskUserQuestion` auto-appends an "Other" free-text input — that is the user's paste slot. State this in the option's `description` (e.g. "Click \"Other\" below and paste the token — I'll write it to .env"). Never tell the user to paste a secret as a chat message when a clickable picker is on screen. After receiving the pasted value via Other, write it to `.env` (replace existing line or append), never echo it back, then re-run the stage check.
