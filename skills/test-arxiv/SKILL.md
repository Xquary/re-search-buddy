...
End-to-end test: arXiv MCP search → API embedding → cosine similarity ranking → xlsx export → PDF download. Runs ``skills/test-arxiv/test_arxiv_pipeline.py``.
...
PYTHONPATH=src .venv/bin/python skills/test-arxiv/test_arxiv_pipeline.py --input "my_file"

# Year filter + custom counts
PYTHONPATH=src .venv/bin/python skills/test-arxiv/test_arxiv_pipeline.py --year-from 2020 --year-to 2025 --max-per-query 20 --top-n 15
```
...
Same 5 queries as `skills/test-scholar/test_pipeline.py` (agent-based modeling, green steel, supply chain). If the TEXT has changed, update QUERIES in the script.
