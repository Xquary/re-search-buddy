...
End-to-end test: translate English queries to Chinese → CNKI Chrome DevTools MCP search → API embedding → cosine similarity ranking → xlsx export. Runs ``skills/test-cnki/test_cnki_pipeline.py``.
...
PYTHONPATH=src .venv/bin/python skills/test-cnki/test_cnki_pipeline.py

# With cached input embedding
PYTHONPATH=src .venv/bin/python skills/test-cnki/test_cnki_pipeline.py --input "my_file"

# With year filter
PYTHONPATH=src .venv/bin/python skills/test-cnki/test_cnki_pipeline.py --year-from 2020 --year-to 2025
```
