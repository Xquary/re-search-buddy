...
Verifies that each MCP server backend starts, lists tools, and handles a test call. Runs ``skills/test-mcp/test_mcp.py`` in the project root.
...
PYTHONPATH=src .venv/bin/python skills/test-mcp/test_mcp.py

# Test a single backend
PYTHONPATH=src .venv/bin/python skills/test-mcp/test_mcp.py arxiv
PYTHONPATH=src .venv/bin/python skills/test-mcp/test_mcp.py scholar
PYTHONPATH=src .venv/bin/python skills/test-mcp/test_mcp.py cnki
```
...
File: `skills/test-mcp/test_mcp.py` — 90 lines, uses `asyncio.run(main())`, no argparse flags.
