"""End-to-end test: Scopus search → embed → rank → output top N.
...
parser.add_argument("--max-per-query", dest="max_per_query", type=int, default=25)
parser.add_argument("--top-n", dest="top_n", type=int, default=10)
parser.add_argument("--query", dest="query_override", default=None, help="Use a single query instead of LLM extraction")
parser.add_argument("--no-download", dest="no_download", action="store_true", help="Skip PDF download step")
...
cfg.setdefault("search", {}).setdefault("scopus", {})["max_results"] = _args.max_per_query
...
if _args.query_override:
    QUERIES = [_args.query_override]
else:
...
print(f"Found {len(papers)} unique papers from Scopus")
