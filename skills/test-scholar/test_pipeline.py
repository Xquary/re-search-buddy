"""End-to-end test: Scholar search → embed → rank → output top 10.

Bypasses the LLM keyword extractor with hand-crafted queries.

Optional: pass ``--input <name>`` (e.g. ``--input green_steel.md``) to load a
pre-computed input embedding from ``input/embeddings/`` instead of re-embedding
the hardcoded ``TEXT`` below. Run ``scan_input.py`` first to populate the store.
"""
import argparse
import __future__
...
cfg.setdefault("search", {}).setdefault("scholar", {})["max_results"] = _args.max_per_query
...
csv_path = search_dir / f"{search_dir_name}.csv"
