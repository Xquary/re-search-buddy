...
Tests the input store pipeline: scan `input/raw/` for new or changed files → extract plain text (PDF/DOCX/MD/TXT) → embed via configured provider → cache `.npy` vectors. Runs ``scan_input.py``.
...
PYTHONPATH=src .venv/bin/python scan_input.py

# List currently cached inputs (no processing)
PYTHONPATH=src .venv/bin/python scan_input.py --list

# Scan but keep manifest entries for deleted raw files
PYTHONPATH=src .venv/bin/python scan_input.py --no-prune
```
...
| `load()` refuses vector | Model mismatch (`manifest.json` model ≠ current config) | Re-run `scan_input.py` (auto re-embeds) |
...
```bash
PYTHONPATH=src .venv/bin/python scan_input.py              # embed first
PYTHONPATH=src .venv/bin/python test_pipeline.py --input paper1  # then use
```
