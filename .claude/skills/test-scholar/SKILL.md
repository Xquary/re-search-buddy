...
End-to-end test: Google Scholar search → API embedding → cosine similarity ranking → xlsx export → PDF download. Runs ``test_pipeline.py``.
...
PYTHONPATH=src .venv/bin/python test_pipeline.py

# With cached input embedding
PYTHONPATH=src .venv/bin/python test_pipeline.py --input "my_file"

# With year filter and custom counts
PYTHONPATH=src .venv/bin/python test_pipeline.py --year-from 2020 --year-to 2025 --max-per-query 20 --top-n 15
```
...
The script uses 5 queries about agent-based modeling, green steel, supply chain. Before running, review the QUERIES list in `test_pipeline.py` — if the TEXT has changed and queries no longer match, propose new queries.
